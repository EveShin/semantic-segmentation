...
class ResNet(nn.Module):
    def __init__(self, block, layers):
        ...
        self.layer1 = self._make_layer(block, 64, layers[0], 1, 1) # layer block 1
        self.layer2 = self._make_layer(block, 128, layers[1], 2, 1) # layer block 2

        # Layer 3, 4는 stride 1로 해상도 유지, dilation 적용해서 수용 영역 늘림
        self.layer3 = self._make_layer(block, 256, layers[2], 1, 2) # layer block 3
        self.layer4 = self._make_layer(block, 512, layers[3], 1, 4) # layer block 4

    def _make_layer(self, block, out_channel, blocks, stride = 1, dilation = 1):
        # 연속된 BasicBlock 또는 Bottleneck block 생성
        ...
        layers.append(block(self.in_channels, out_channel, stride, dilation, downsample)) # 블록 1 : downsample 인자도 넘겨줌
        self.in_channels= out_channel * block.expansion

        for i in range(1, blocks):
            layers.append(block(self.in_channels, out_channel, dilation = dilation)) # 나머지 블록
    ...
    def forward(self, x):
        ...
        x = self.layer1(x) # basic block : 3x3 -> 3x3      bottleneck : 1x1 -> 3x3 -> 1x1
        x = self.layer2(x)  # basic block : 3x3 -> 3x3      bottleneck : 1x1 -> 3x3 -> 1x1

        # x_aux : 보조 손실 계산(Layer3 끝)    x_main : 메인 예측용(Layer4 끝)
        x_aux = self.layer3(x) # basic block : 3x3 -> 3x3      bottleneck : 1x1 -> 3x3 -> 1x1
        x_main = self.layer4(x_aux) # basic block : 3x3 -> 3x3      bottleneck : 1x1 -> 3x3 -> 1x1
